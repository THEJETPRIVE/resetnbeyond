  ...
  return (
    <Link href={`/resorts/${resort.slug}`} className={cn("group block", className)}>
      <div className={cn("relative overflow-hidden rounded-[2px]", aspect)}>
        ...
        <span className="absolute bottom-4 left-4 z-[3] overline text-ivory/80">{resort.country}</span>
      </div>

      <div className="mt-6 flex flex-col gap-2">
        <div className="flex items-baseline justify-between gap-4">
          <h3 className="font-serif text-3xl font-light text-foreground transition-colors group-hover:text-sage-deep dark:group-hover:text-sage">
            {resort.name}
          </h3>
        </div>
